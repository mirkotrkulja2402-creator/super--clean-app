  ...
  async (request, response, next) => {
    try {
      ...
      if (!code || code === "ADMIN") {
        return response
          .status(400)
          .json({ error: "Naziv uloge nije dozvoljen." });
      }
  ...
  async (request, response, next) => {
    try {
      ...
      if (users.rows[0].count > 0) {
        return response.status(409).json({
          error:
            "Uloga se koristi kod korisnika. Prvo promijeni njihove uloge.",
        });
      }
...
function normalizeOrderItemInput(body) {
  return {
    ...
    unit: String(body.unit || "")
      ...
      .toLowerCase(),
    lengthM:
      body.lengthM === "" || body.lengthM == null ? null : Number(body.lengthM),
    widthM:
      body.widthM === "" || body.widthM == null ? null : Number(body.widthM),
...
function validateOrderItemInput(input) {
  ...
  if (!["m2", "piece"].includes(input.unit))
    return "Jedinica naplate nije ispravna.";
  if (
...
function publicOrder(row) {
  return {
    ...
    invoiceNumber:
      row.invoice_no === null || row.invoice_no === undefined
        ? null
  ...
  async (request, response, next) => {
    try {
      ...
      if (error.code === "23505") {
        return response
          .status(409)
          .json({ error: "Ova usluga već postoji za izabranu jedinicu." });
      }
      return next(error);
    }
  }
  ...
  async (request, response, next) => {
    try {
      ...
      if (result.rowCount !== 1) {
        return response
          .status(404)
          .json({ error: "Stavka cjenovnika nije pronađena." });
      }
...
app.get(
  "/api/qr/order/:orderId/item/:itemId",
  authenticate,
  ...
  async (request, response, next) => {
    try {
      ...
      if (result.rowCount !== 1) {
        return response
          .status(404)
          .json({ error: "Stavka za QR kod nije pronađena." });
      }

      const baseUrl = `${request.protocol}://${request.get("host")}`;
      const targetUrl = `${baseUrl}/?order=${encodeURIComponent(
        result.rows[0].order_id
      )}&item=${encodeURIComponent(result.rows[0].item_id)}`;
...
app.post(
  "/api/qr/order/:orderId/item/:itemId/scan",
  authenticate,
  requirePermission("qr.view"),
  async (request, response, next) => {
    try {
  ...
  async (request, response, next) => {
    try {
      ...
      if (customerResult.rowCount !== 1) {
        return response
          .status(400)
          .json({ error: "Kupac nije pronađen ili nije aktivan." });
      }
...
app.post(
  ...
  async (request, response, next) => {
    try {
      ...
      return response
        .status(201)
        .json({ item: publicOrderItem(result.rows[0]) });
    } catch (error) {
      next(error);
    }
  }
  ...
  async (request, response, next) => {
    try {
      ...
      if (result.rowCount !== 1) {
        return response
          .status(404)
          .json({ error: "Stavka narudžbe nije pronađena." });
      }
      return response.status(204).end();
    } catch (error) {
      next(error);
  ...
  async (request, response, next) => {
    ...
    try {
      ...
      const dueDate = request.body?.dueDate
        ? String(request.body.dueDate).trim()
        : null;
      ...
      if (!["cash", "card", "bank"].includes(paymentMethod)) {
        return response
          .status(400)
          .json({ error: "Način plaćanja nije ispravan." });
      }
      if (
        paymentMethod === "bank" &&
        ...
        return response
          .status(400)
          .json({ error: "Za žiralno plaćanje unesite datum dospijeća." });
      }
      ...
      if (incompleteItem) {
        ...
        return response.status(400).json({
          error:
            "Račun se ne može završiti dok svaki tepih nema dimenzije ili oznaku „Izmjeren prilikom preuzimanja“.",
      ...
      if (existing.rowCount === 1) {
        ...
        if (invoice.status === "CANCELLED") {
          return response
            .status(409)
            .json({
              error: "Ovaj račun je storniran i ne može se ponovo fakturisati.",
            });
        }
      ...
      const result = await client.query(
        ...
        [
          ...
          paymentMethod,
          request.user.id,
          paymentMethod === "bank" ? dueDate : null,
        ]
      );
  ...
  async (request, response, next) => {
    try {
      ...
      if (result.rowCount !== 1) {
        return response
          .status(404)
          .json({ error: "Račun nije pronađen ili je već storniran." });
      }
...
app.post(
  "/api/routes/jobs/from-order/:orderId",
  authenticate,
  routePermission("routes.assign"),
  async (request, response, next) => {
    try {
      ...
      const result = await pool.query(
        ...
        [
          request.params.orderId,
          order.rows[0].latitude,
          order.rows[0].longitude,
        ]
      );
...
app.post(
  "/api/routes/jobs/from-invoice/:invoiceId",
  authenticate,
  routePermission("routes.assign"),
  async (request, response, next) => {
    try {
      ...
      if (invoice.rowCount !== 1)
        return response.status(404).json({ error: "Račun nije pronađen." });
      const result = await pool.query(
        `INSERT INTO route_jobs
                (source_type, source_id, action_type, latitude, longitude)
             VALUES ('INVOICE', $1, 'DELIVERY', $2, $3)
        ...
        [
          request.params.invoiceId,
          invoice.rows[0].latitude,
          invoice.rows[0].longitude,
        ]
      );
      return response.status(201).json({ route: result.rows[0] });
    } catch (error) {
  ...
  async (request, response, next) => {
    try {
      ...
      if (!name || name.length > 100) {
        return response
          .status(400)
          .json({ error: "Naziv vozila nije ispravan." });
      }
      const result = await pool.query(
  ...
  async (request, response, next) => {
    try {
      ...
      if (vehicle.rowCount !== 1) {
        return response
          .status(400)
          .json({ error: "Vozilo nije aktivno ili ne postoji." });
      }
      const result = await pool.query(
        `UPDATE route_jobs
      ...
      if (result.rowCount !== 1) {
        return response
          .status(404)
          .json({ error: "Vožnja nije pronađena ili je već dodijeljena." });
      }
  ...
  async (request, response, next) => {
    ...
    try {
      ...
      if (!Number.isInteger(sequence) || sequence < 1) {
        return response
          .status(400)
          .json({ error: "Redoslijed nije ispravan." });
      }
      ...
      if (current.rowCount !== 1) {
        ...
        return response
          .status(404)
          .json({ error: "Stavka rute nije pronađena." });
      }
      ...
      await client.query("COMMIT");
      return response.json({
        route: { id: request.params.id, sequence_no: targetSequence },
      });
    } catch (error) {
      await client.query("ROLLBACK").catch(() => {});
      return next(error);
  ...
  async (request, response, next) => {
    ...
    try {
      ...
      if (current.rowCount !== 1) {
        ...
        return response
          .status(404)
          .json({ error: "Vožnja nije pronađena u ruti vozila." });
      }
      await client.query(
  ...
  async (request, response, next) => {
    try {
      ...
      if (result.rowCount !== 1) {
        return response
          .status(404)
          .json({ error: "Vožnja nije pronađena u aktivnoj ruti." });
      }
      return response.json({ route: result.rows[0] });
    } catch (error) {
      return next(error);
  ...
  async (request, response, next) => {
    try {
      ...
      if (
        ...
        return response.status(400).json({
          error:
            "Koordinate baze Super Clean nisu podešene. Ruta nije automatski mijenjana.",
        });
      }
      ...
      return response.json({
        vehicleId,
        optimized: ordered.map((job, index) => ({
          id: job.id,
          sequence: index + 1,
        })),
      });
...
app.post(
  "/api/cashier/:invoiceId/pay",
  authenticate,
  routePermission("cashier.edit"),
  async (request, response, next) => {
    try {
      const method = String(request.body?.paymentMethod || "")
        .trim()
        .toLowerCase();
...
app.put(
  ...
  async (request, response, next) => {
    try {
      ...
      const latitude =
        body.baseLatitude === "" || body.baseLatitude == null
          ? null
          : Number(body.baseLatitude);
      const longitude =
        body.baseLongitude === "" || body.baseLongitude == null
          ? null
          : Number(body.baseLongitude);
      if (
        ...
        return response
          .status(400)
          .json({ error: "Koordinate baze nisu ispravne." });
      }
      ...
      return response.json({
        memorandum,
        baseCoordinates: { latitude, longitude },
      });
    } catch (error) {
      return next(error);
    }
  ...
  async (request, response, next) => {
    try {
      ...
      if (result.rowCount !== 1) {
        return response
          .status(404)
          .json({ error: "Naplaćivanje nije pronađeno." });
      }
      return response.json({ payment: result.rows[0] });
    } catch (error) {
      return next(error);
...
const BACKUP_TABLES = [
  [
    "roles",
    "SELECT code, name, description, is_system, created_at, updated_at FROM roles ORDER BY code",
  ],
  [
    "permissions",
    "SELECT code, name, module, created_at FROM permissions ORDER BY code",
  ],
  [
    "role_permissions",
  ...
  [
    "users",
    "SELECT id, username, first_name, last_name, role, is_active, created_at, updated_at FROM users ORDER BY username",
  ],
  [
    "customers",
    "SELECT id, first_name, last_name, company_name, phone, whatsapp, address, city, note, latitude, longitude, is_active, created_at, updated_at FROM customers ORDER BY created_at, id",
  ],
  [
    "orders",
    "SELECT id, order_number, customer_id, order_date, planned_pickup_date, pickup_time_from, pickup_time_to, actual_pickup_at, status, note, created_by, delivery_requested, delivery_price, created_at, updated_at FROM orders ORDER BY order_number",
  ],
  [
    "order_items",
    "SELECT id, order_id, service_name, unit, length_m, width_m, quantity, unit_price, area_m2, total, note, measured_at_pickup, qr_scan_count, last_qr_scanned_at, created_at, updated_at FROM order_items ORDER BY created_at, id",
  ],
  [
    "price_list",
    "SELECT id, service_name, unit, price, is_active, sort_order, created_at, updated_at FROM price_list ORDER BY sort_order, service_name",
  ],
  [
    "invoices",
    "SELECT id, order_id, invoice_no, status, payment_method, issued_at, due_date, cancelled_at, created_by, created_at, updated_at FROM invoices ORDER BY invoice_no",
  ],
  [
    "vehicles",
    "SELECT id, name, is_active, sort_order, created_at, updated_at FROM vehicles ORDER BY sort_order, name",
  ],
  [
    "route_jobs",
    "SELECT id, source_type, source_id, action_type, vehicle_id, sequence_no, status, latitude, longitude, assigned_at, assigned_by, completed_at, last_attempt_at, last_attempt_by, attempt_count, note, created_at, updated_at FROM route_jobs ORDER BY created_at, id",
  ],
  [
    "route_attempts",
    "SELECT id, route_job_id, vehicle_id, user_id, result, created_at FROM route_attempts ORDER BY id",
  ],
  [
    "invoice_payments",
    "SELECT invoice_id, paid, payment_method, paid_at, paid_by FROM invoice_payments ORDER BY invoice_id",
  ],
  [
    "app_settings",
    "SELECT setting_key, setting_value, updated_by, updated_at FROM app_settings ORDER BY setting_key",
  ],
];
...
async function removeOldBackups() {
  ...
  for (const row of result.rows) {
    try {
      ...
      if (error.code !== "ENOENT")
        console.error("Brisanje starog backupa nije uspjelo:", error.message);
    }
  ...
  for (const filename of files) {
    if (
      !filename.startsWith("super-clean-backup-") ||
...
function validateBusinessBackupPayload(payload) {
  if (
    !payload ||
    payload.format !== "super-clean-business-backup" ||
...
async function runSystemSelfTest() {
  ...
  try {
    ...
    const qrTarget = `http://test.local/?order=${encodeURIComponent(
      order.rows[0].id
    )}&item=${encodeURIComponent(item.rows[0].id)}`;
    ...
    if (total !== 40) {
      throw new Error(
        `Test obračuna nije prošao: očekivano 40.00 KM, dobijeno ${total.toFixed(
          2
        )} KM.`
      );
    }
...
async function restoreBusinessBackup(payload) {
  ...
  const restoreTables = restoreOrder.map((table) => `"${table}"`).join(", ");
  ...
  try {
    ...
    for (const table of restoreOrder) {
      ...
      const tableColumns = tableDefinitions[table];
      const quotedColumns = tableColumns
        .map((column) => `"${column}"`)
        .join(", ");
      const placeholders = tableColumns
        .map((_, index) => `$${index + 1}`)
        .join(", ");

      for (const row of rows) {
        if (!row || typeof row !== "object" || Array.isArray(row)) {
          throw new Error(`Backup sadrži neispravan zapis u tabeli ${table}.`);
        }
    ...
    await client.query(`
            SELECT setval(
                pg_get_serial_sequence('route_attempts', 'id'),
                GREATEST(COALESCE((SELECT MAX(id) FROM route_attempts), 1), 1),
                true
            )
        `);
  ...
  async (request, response, next) => {
    try {
      ...
      const backupAgeHours = lastBackup
        ? Math.floor(
            (Date.now() - new Date(lastBackup.created_at).getTime()) / 3600000
          )
        : null;
  ...
  async (request, response, next) => {
    try {
      ...
      try {
        ...
        return response
          .status(400)
          .json({ error: "Backup datoteka nije ispravan JSON." });
      }
...
app.get(
  ...
  async (request, response, next) => {
    try {
      ...
      const canViewCustomers = await requestHasPermission(
        request,
        "customers.view"
      );
      const canViewInvoices = await requestHasPermission(
        request,
        "invoices.view"
      );
      const canViewPricing = await requestHasPermission(
        request,
        "pricing.view"
      );
      ...
      if (canViewOrders) {
        ...
        if (canViewInvoices) {
          warnings.push({
            ...
            description:
              "Narudžba ima mjerenje, ali račun još nije napravljen.",
            page: "orders",
          });
        }
      }
      ...
      return response.json({
        ...
        summary: {
          ...
          newCustomerCount: canViewCustomers
            ? Number(row.new_customer_count)
            : null,
        },
      ...
      for (const row of orders.rows) {
        results.push({
          ...
          subtitle: `${row.customer_name} · ${
            row.phone || "bez telefona"
          } · ${formatDateForSearch(row.order_date)}`,
        });
      }
    }
    ...
    if (await requestHasPermission(request, "customers.view")) {
      ...
      for (const row of customers.rows) {
        results.push({
          ...
          subtitle: `${row.phone || "bez telefona"}${
    ...
    if (await requestHasPermission(request, "invoices.view")) {
      ...
      for (const row of invoices.rows) {
        results.push({
          ...
          subtitle: `N${row.order_number || ""} · ${row.customer_name} · ${
            row.status === "CANCELLED" ? "Storniran" : "Fakturisan"
          }`,
        });
      }
    }
  ...
  async (request, response, next) => {
    try {
      ...
      if (
        ...
        return response
          .status(400)
          .json({ error: "Period izvještaja nije ispravan." });
      }

      if (from > to) {
        return response
          .status(400)
          .json({ error: "Početni datum ne može biti nakon završnog." });
      }
  ...
  async (request, response, next) => {
    try {
      ...
      if (address.length < 3) {
        return response
          .status(400)
          .json({ error: "Unesi adresu prije pretrage." });
      }
      ...
      if (error.name === "TimeoutError" || error.name === "AbortError") {
        return response
          .status(504)
          .json({ error: "Pretraga adrese je istekla. Pokušaj ponovo." });
      }
      return next(error);
    }
  }
...
async function ensureInvoicesTable() {
  await pool.query(`
        CREATE TABLE IF NOT EXISTS invoices (
            ...
            invoice_no BIGSERIAL UNIQUE NOT NULL,
