...
function createSessionCookie(token, days = SESSION_DAYS) {
    const secure = process.env.NODE_ENV === "production" ? "; Secure" : "";
    const maxAge = Number.isFinite(days) && days > 0 ? `; Max-Age=${days * 86400}` : "";
    return `session=${encodeURIComponent(token)}; HttpOnly; SameSite=Lax; Path=/${maxAge}${secure}`;
}
...
app.post("/api/login", loginLimiter, async (request, response, next) => {
    try {
        ...
        const rememberDevice = request.body.rememberDevice !== false;
        ...
        response.setHeader("Set-Cookie", createSessionCookie(token, rememberDevice ? SESSION_DAYS : null));
...
function normalizeLoginSettings(value = {}) {
    const result = { ...LOGIN_SCREEN_DEFAULTS, ...(value && typeof value === "object" ? value : {}) };
    const hex = /^#[0-9a-fA-F]{6}$/;
    for (const key of ["titleColor", "textColor", "accentColor", "backgroundColor", "cardColor"]) {
        if (!hex.test(String(result[key] || ""))) result[key] = LOGIN_SCREEN_DEFAULTS[key];
    }
    result.logoSrc = typeof result.logoSrc === "string" && result.logoSrc.startsWith(("data:image/")) &&
        result.logoSrc.length <= 450000 ? result.logoSrc : (
        typeof result.logoSrc === "string" && result.logoSrc.startsWith("/") ? result.logoSrc : LOGIN_SCREEN_DEFAULTS.logoSrc
    );
    for (const key of ["rememberVisible", "rememberDefault", "changeUserVisible", "passwordToggleVisible",
...
app.get("/api/login-settings", async (request, response, next) => {
    try {
        const result = await pool.query(
            `SELECT setting_value FROM app_settings WHERE setting_key = 'login.screen'`,
        );
        response.json({
            settings: normalizeLoginSettings(result.rows[0]?.setting_value || {}),
        });
    } catch (error) {
        next(error);
    }
...
app.get("/api/admin/login-settings", authenticate, requireAdmin, async (request, response, next) => {
    try {
        const result = await pool.query(
            `SELECT setting_value, updated_at
             FROM app_settings
             WHERE setting_key = 'login.screen'`,
        );
        response.json({
            settings: normalizeLoginSettings(result.rows[0]?.setting_value || {}),
...
app.put("/api/admin/login-settings", authenticate, requireAdmin, async (request, response, next) => {
    try {
        const settings = normalizeLoginSettings(request.body?.settings || {});
        await pool.query(
            `INSERT INTO app_settings (setting_key, setting_value, updated_by)
             VALUES ('login.screen', $1::jsonb, $2)
...
app.post("/api/admin/login-settings/reset", authenticate, requireAdmin, async (request, response, next) => {
    try {
        await pool.query(
            `DELETE FROM app_settings WHERE setting_key = 'login.screen'`,
        );
        response.json({ settings: LOGIN_SCREEN_DEFAULTS });
    } catch (error) {
        next(error);
    }
});
...
async function restoreBusinessBackup(payload) {
    ...
    const tableDefinitions = {
        ...
        orders: ["id", "order_number", "customer_id", "order_date", "planned_pickup_date", "pickup_time_from", "pickup_time_to", "actual_pickup_at", "status", "note", "created_by", "delivery_requested", "delivery_price", "created_at", "updated_at"],
        order_items: ["id", "order_id", "service_name", "unit", "length_m", "width_m", "quantity", "unit_price", "area_m2", "total", "note", "measured_at_pickup", "qr_scan_count", "last_qr_scanned_at", "created_at", "updated_at"],
        ...
        vehicles: ["id", "name", "is_active", "sort_order", "created_at", "updated_at"],
        route_jobs: ["id", "source_type", "source_id", "action_type", "vehicle_id", "sequence_no", "status", "latitude", "longitude", "assigned_at", "assigned_by", "completed_at", "last_attempt_at", "last_attempt_by", "attempt_count", "note", "created_at", "updated_at"],
        route_attempts: ["id", "route_job_id", "vehicle_id", "user_id", "result", "created_at"],
        invoice_payments: ["invoice_id", "paid", "payment_method", "paid_at", "paid_by"],
