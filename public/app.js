...
(function ensureStableMobileViewport() {
...
function scIcon(name, className = "") {
    const icons = {
        menu: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h16"/></svg>',
        home: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 10.8 12 3l9 7.8"/><path d="M5.5 9.8V21h13V9.8M9 21v-6h6v6"/></svg>',
        orders: '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="6" y="3.5" width="12" height="17" rx="2"/><path d="M9 7h6M9 11h6M9 15h4"/></svg>',
        customers: '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><circle cx="16.5" cy="9.5" r="2.5"/><path d="M3.5 20c.5-3.4 2.4-5 5.5-5s5 1.6 5.5 5M14 15c3 0 5 1.5 6 5"/></svg>',
        price: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 6.5 13.5 4 20 10.5 10.5 20 4 13.5z"/><circle cx="8.2" cy="8.2" r="1"/></svg>',
        label: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 4 14 0v16H5z"/><path d="M8 8h8M8 12h8M8 16h5"/></svg>',
        qr: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM15 14h2v2h-2zM18 17h2v3h-3v-2M14 18h2v2h-2z"/></svg>',
        invoice: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3h9l4 4v14H6z"/><path d="M15 3v5h4M9 12h6M9 16h6"/></svg>',
        reports: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20V4M4 20h16"/><path d="m7 15 3-4 3 2 5-7"/></svg>',
        ...
        eyeOff: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3l18 18"/><path d="M10.6 6.2A10.7 10.7 0 0 1 12 6c6 0 9.5 6 9.5 6a17.7 17.7 0 0 1-3.1 3.9M6.7 6.7C4 8.3 2.5 12 2.5 12s3.5 6 9.5 6a10.6 10.6 0 0 0 3.4-.6"/><path d="M9.9 9.9a3 3 0 0 0 4.2 4.2"/></svg>',
        userSwitch: '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><path d="M3.5 20c.5-3.4 2.4-5 5.5-5 2.2 0 3.8.8 4.8 2.5"/><path d="M15 7h6M18 4l3 3-3 3"/></svg>',
        print: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 9V4h10v5M7 18H5a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><path d="M7 14h10v7H7z"/></svg>',
        pdf: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3h9l4 4v14H6z"/><path d="M15 3v5h4M8.5 16h2.2a1.5 1.5 0 0 0 0-3H8.5v5M13 18v-5h1.5a2.5 2.5 0 0 1 0 5H13"/></svg>',
        ...
        admin: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m12 3 1.4 1.9 2.3-.1.8 2.1 2 .9-.6 2.2 1.5 1.7-1.5 1.7.6 2.2-2 .9-.8 2.1-2.3-.1L12 21l-1.4-1.9-2.3.1-.8-2.1-2-.9.6-2.2L4.6 12l1.5-1.7-.6-2.2 2-.9.8-2.1 2.3.1z"/><circle cx="12" cy="12" r="3"/></svg>',
        map: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5.5 9 3l6 3 5-2.5v15L15 21l-6-3-5 2.5z"/><path d="M9 3v15M15 6v15"/></svg>',
        ...
        quick: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m13 2-9 12h7l-1 8 9-12h-7z"/></svg>',
        gari: '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="5" width="16" height="14" rx="2"/><path d="M8 9h8M8 13h5"/></svg>'
    };
...
function loginView(error = "") {
    ...
    document.getElementById("password-toggle")?.addEventListener("click", () => {
        ...
        button.innerHTML = visible ? scIcon("eye") : scIcon("eyeOff");
...
function dashboardView(user) {
    ...
    const menu = [
        ["home", "Početna", "home", null],
        ["orders", "Narudžbe", "orders", "orders.view"],
        ["customers", "Kupci", "customers", "customers.view"],
        ["price", "Cjenovnik", "pricing", "pricing.view"],
        ["label", "Etikete", "labels", "labels.view"],
        ["qr", "QR kod", "qr", "qr.view"],
        ["invoice", "Računi", "invoices", "invoices.view"],
        ["reports", "Izvještaji", "reports", "reports.view"],
    ];

    const sidebarItems = menu
        .filter(([, , , permission]) => !permission || hasPermission(permission))
        .map(([icon, title, page]) => `
            <button class="sc-sidebar-item${page === "home" ? " active" : ""}" type="button" data-page="${page}">
                ${scIcon(icon)}
    ...
    const adminSidebar = user.role === "ADMIN" ? `
        <div class="sc-sidebar-divider"></div>
        <button class="sc-sidebar-item" type="button" data-page="admin">
            ${scIcon("admin")}
    ...
    app.innerHTML = `
        <div class="sc-home-app">
            <header class="sc-app-header">
                <button class="sc-header-menu" id="menu-toggle" type="button" aria-label="Meni">${scIcon("menu")}</button>
                <div class="sc-header-brand">
                    <img src="/logo-login.png" alt="Super Clean Banja Luka">
                    <div><strong>Super Clean</strong><small>Banja Luka</small></div>
                </div>

                <label class="sc-header-search">
                    ${scIcon("search")}
                    <input id="global-search" placeholder="Pretraži..." aria-label="Pretraži">
                </label>

                <div class="sc-header-actions">
                    <button type="button" data-action="search" aria-label="Pretraga">${scIcon("search")}</button>
                    <button type="button" id="user-menu" aria-label="Korisnik">${scIcon("customers")}</button>
                    <button type="button" class="sc-user-name" id="user-menu-text">
                        <span>${escapeHtml(user.firstName || "Korisnik")}</span>
                        <small>${escapeHtml(user.roleName || user.role)}</small>
                    </button>
                </div>
            </header>

            <aside class="sc-sidebar" aria-label="Glavni meni">
                <div class="sc-sidebar-logo">
                    <img src="/logo-login.png" alt="Super Clean Banja Luka">
                </div>
                <nav>${sidebarItems}${adminSidebar}</nav>
            </aside>
            ...
            <nav class="sc-mobile-bottom-nav">
                <button type="button" data-page="home" class="active">${scIcon("home")}<span>Početna</span></button>
                ${hasPermission("orders.view") ? `<button type="button" data-page="orders">${scIcon("orders")}<span>Narudžbe</span></button>` : ""}
                ${hasPermission("customers.view") ? `<button type="button" data-page="customers">${scIcon("customers")}<span>Kupci</span></button>` : ""}
                ${user.role === "ADMIN" ? `<button type="button" data-page="admin">${scIcon("admin")}<span>Admin</span></button>` : ""}
            </nav>
...
function homeContent() {
    const items = [
        ["orders", "Nova narudžba", "Brzo zaprimanje tepiha", "orders.view", "orders"],
        ["customers", "Kupci", "Kupci i kontakt podaci", "customers.view", "customers"],
        ["orders", "Narudžbe", "Pregled i statusi", "orders.view", "orders"],
        ["invoice", "Računi", "Računi i fakture", "invoices.view", "invoices"],
        ["reports", "Izvještaji", "Pregled poslovanja", "reports.view", "reports"],
        ["price", "Cjenovnik", "Cijene i usluge", "pricing.view", "pricing"],
        ["label", "Etikete", "Evidencija i štampa etiketa", "labels.view", "labels"],
        ["qr", "QR kod", "QR alati", "qr.view", "qr"],
    ].filter(([, , , permission]) => hasPermission(permission));

    if (currentUser.role === "ADMIN") {
        items.push(["admin", "Administrator", "Upravljanje aplikacijom", null, "admin"]);
    }

    const cards = items.map(([icon, title, description, , page]) => `
        <button class="sc-home-card" type="button" data-page="${page}" data-page-label="${escapeHtml(title)}">
            <span class="sc-home-card-icon">${scIcon(icon)}</span>
            <span class="sc-home-card-content">
                <strong>${escapeHtml(title)}</strong>
                <small>${escapeHtml(description)}</small>
            </span>
            <span class="sc-home-card-arrow">→</span>
        </button>
    `).join("");
    ...
    return `
        <div class="sc-home-page">
            <section class="sc-home-hero">
                <div class="sc-home-welcome">
                    <span>DOBRO DOŠAO</span>
                    <h1>${firstName} <b>👋</b></h1>
                    <div class="sc-role-badge">${scIcon("admin")} ${roleLabel}</div>
                </div>
                <img class="sc-home-hero-logo" src="/logo-login.png" alt="Super Clean Banja Luka">
            </section>

            <button class="sc-home-new-order" type="button" data-page="orders" data-page-label="Nova narudžba">
                <span class="sc-home-new-icon">${scIcon("plus")}</span>
                <span class="sc-home-new-copy">
                    <small>NOVA NARUDŽBA</small>
                    <strong>Brz prijem novog tepiha</strong>
                </span>
                <b>→</b>
            </button>

            <section class="sc-main-menu-section">
                <span class="sc-main-eyebrow">GLAVNI MENI</span>
                <h2>Šta želiš otvoriti?</h2>
                <div class="sc-home-grid">${cards}</div>
            </section>

            <footer class="sc-home-footer">Čist dom <i>•</i> Zdraviji život</footer>
        </div>
    `;
}
...
function customersContent(customers = [], search = "") {
    ...
    const rows = customers.map((customer) => {
        ...
        return `
            <article class="customer-card selectable-record" data-customer-id="${escapeHtml(customer.id)}" data-record-type="customer" data-record-id="${escapeHtml(customer.id)}">
...
function renderUsers(users) {
    const rows = users.map((user) => `
        <article class="user-card selectable-record" data-record-type="user" data-record-id="${escapeHtml(user.id)}">
...
function renderRoles() {
    const roleCards = rolesData.map((role) => `
        <article class="role-card selectable-record" data-record-type="role" data-record-id="${escapeHtml(role.code)}">
...
async function api(url, options = {}) {
    ...
    if (text) {
        try {
            ...
            throw new Error("Server je vratio neispravan odgovor.");
        }
    }

    if (!response.ok) {
        throw new Error(data.error || `Operacija nije uspjela (${response.status}).`);
    }
...
function ordersContent(orders = [], filters = {}) {
    ...
    const rows = orders.map((order) => `
        <article class="order-card selectable-record" data-order-id="${escapeHtml(order.id)}" data-record-type="order" data-record-id="${escapeHtml(order.id)}">
    ...
    return `
        <section class="page-panel">
            ...
            <div class="pricing-list">
                ${prices.map((item) => `
                    <article class="pricing-card selectable-record ${item.isActive ? "" : "is-inactive"}" data-record-type="pricing" data-record-id="${escapeHtml(item.id)}">
                        <div>
                            <strong>${escapeHtml(item.serviceName)}</strong>
...
function navigateByLabel(label) {
    ...
    if (label === "Narudžbe" || label === "Nova narudžba" || label === "Brzi unos") return navigate("orders");
    ...
    document.getElementById("page-content").querySelector("[data-go-home]")?.addEventListener("click", () => navigate("home"));
}
    ...
    if (currentPage === "invoices") {
        ...
        downloadCsv("super-clean-racuni.csv", [
            ...
            ...data.invoices.map((order) => [
                ...
                Number((Number(order.itemsTotal || 0) + Number(order.deliveryPrice || 0))).toFixed(2),
            ]),
        ]);
        return;
    }
...
async function handleToolbarAction(action) {
    ...
    if (action === "edit") {
        ...
        if (selectedRecord.type === "customer") {
            document.querySelector(`.edit-customer[data-customer-id="${CSS.escape(selectedRecord.id)}"]`)?.click();
            return;
        }
        if (selectedRecord.type === "order") {
            document.querySelector(`.edit-order[data-order-id="${CSS.escape(selectedRecord.id)}"]`)?.click();
            return;
        }
        if (selectedRecord.type === "pricing") {
            document.querySelector(`.edit-price[data-price-id="${CSS.escape(selectedRecord.id)}"]`)?.click();
            return;
        }
        if (selectedRecord.type === "user") {
            document.querySelector(`.edit-user[data-user-id="${CSS.escape(selectedRecord.id)}"]`)?.click();
            return;
        }
        if (selectedRecord.type === "role") {
            document.querySelector(`.edit-role[data-role-code="${CSS.escape(selectedRecord.id)}"]`)?.click();
            return;
        }
    ...
    if (action === "remove") {
        ...
        if (selectedRecord.type === "customer") {
            document.querySelector(`.delete-customer[data-customer-id="${CSS.escape(selectedRecord.id)}"]`)?.click();
            return;
        }
        if (selectedRecord.type === "order") {
            document.querySelector(`.delete-order[data-order-id="${CSS.escape(selectedRecord.id)}"]`)?.click();
            return;
        }
        if (selectedRecord.type === "pricing") {
            document.querySelector(`.delete-price[data-price-id="${CSS.escape(selectedRecord.id)}"]`)?.click();
            return;
        }
        if (selectedRecord.type === "user") {
            document.querySelector(`.edit-user[data-user-id="${CSS.escape(selectedRecord.id)}"]`)?.click();
...
async function handleLogin(event) {
    ...
    button.querySelector("span")?.replaceChildren(document.createTextNode("PRIJAVLJUJEM..."));
    ...
    try {
        ...
        if (!data?.user) {
            throw new Error("Prijava nije vraćena sa servera.");
        }
        ...
        button.querySelector("span")?.replaceChildren(document.createTextNode("PRIJAVI SE"));
    }
}
